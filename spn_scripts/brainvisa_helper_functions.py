        ...
        if 'label' in v:
            ...
            if v['label'] in sulcus_list:
                ...
                if 'aims_Tmtktri' in v and 'aims_bottom' in v:
                    ...
                    for i in range(bottom_map[0].size()):
...
def compute_spn_and_efi(sulcal_phenotypes, sulci_analyzed, save_path, group_efi, english_labels=True):
    sulci_for_analysis_file = open(sulci_analyzed)
    sulci_for_analysis_contents = sulci_for_analysis_file.read()
    sulci_for_analysis = sulci_for_analysis_contents.splitlines()
    french_abbreviations = sulci_for_analysis[0].split(",")
    english_sulci = sulci_for_analysis[1].split(",")
    sulcal_values_retained = np.zeros((5,len(english_sulci)))
    sulcus_itr = 0
    for sulcus in french_abbreviations:
        for i in range(0,5):
            sulcus_loc = np.where(sulcal_phenotypes.sulcus_label == sulcus)[0][0]
            tmp_sulcal_value = sulcal_phenotypes.iloc[sulcus_loc, 2 + i]
            sulcal_values_retained[i, sulcus_itr] = tmp_sulcal_value
        sulcus_itr+=1
    #check for missing values, which would indicate sulci not measured well automatically and suggest exclusion of subject
    if np.all(~np.isnan(sulcal_values_retained)):
        #now z-score columns
        means = np.mean(sulcal_values_retained, axis=1, keepdims=True)
        std_devs = np.std(sulcal_values_retained, axis=1, keepdims=True)
        z_scores = (sulcal_values_retained - means) / std_devs
        spn = np.corrcoef(z_scores,rowvar=False)
        #compute efi
        all_efis = []
        ##if using a custom set of sulci for analysis, then you can't compute efi with respect
        if sulci_analyzed=="/usr/local/spn_scripts/sulci_for_analysis.txt":
            for i in range(0,40):
                sulc_efi = np.corrcoef(spn[i,], group_efi)[0,1]
                all_efis = np.append(all_efis, sulc_efi)
            if english_labels:
                spn_df = pd.DataFrame(spn, index = english_sulci, columns = english_sulci)
                efi_df = pd.DataFrame(all_efis, index = english_sulci , columns = ["efi"])
            else:
                spn_df = pd.DataFrame(spn, index = french_abbreviations, columns = french_abbreviations)
                efi_df = pd.DataFrame(all_efis, index = french_abbreviations, columns = ["efi"] )
            spn_df.to_csv(save_path + "_spn.csv")
            efi_df.to_csv(save_path + "_efi.csv") 
        else:
            if english_labels:
                spn_df = pd.DataFrame(spn, index = english_sulci, columns = english_sulci)
            else:
        ...
        print("Cannot continue processing subject SPN, not all sulci detected by BrainVISA. Consider reprocessing subject, manually editing subject sulci, excluding subject, or considering a different sulcal parcellation scheme")